...
<head>
  ...
  <style>
    ...
    .feature-card{
      position:absolute; top:50%; left:50%; width:260px; height:340px; transform-style:preserve-3d;
      transform-origin:center center -400px; /* depth ring radius */
      border-radius:18px; padding:16px;
      color:#05313a; box-shadow:0 30px 80px rgba(6,20,28,.14);
      background:linear-gradient(160deg, rgba(255,255,255,.86), rgba(255,255,255,.60));
      border:1px solid rgba(6,182,212,.12);
      transition: transform 900ms cubic-bezier(.2,.8,.2,1), box-shadow .3s ease, background .4s ease;
      user-select:none;
    }
    .feature-card:hover{
      box-shadow:0 36px 100px rgba(6,20,28,.18);
    ...
    .f-icon{
      ...
      background:linear-gradient(135deg,#67e8f9,#38bdf8); color:white;
      box-shadow:0 10px 24px rgba(56,189,248,.25);
    }
    .f-title{ font-weight:800; font-size:1.15rem; margin-top:10px }
    .f-desc{ font-size:.92rem; color:#0f3b43; margin-top:8px }
...
<body>
  ...
  <section id="features-showcase" class="px-4 md:px-6 mt-14">
    ...
    <!-- <div class="feature-stage mt-8">
      <div id="carousel" class="carousel">
        Cards are injected by JS
        <div class="ring-dots"></div>
      </div>
      <div class="carousel-controls mt-[500px]">
    ...
    function buildCards(){
      features.forEach((f, i)=>{
        ...
        card.addEventListener('mousemove', (e)=>{
          ...
          card.style.transform = `translate(-50%,-50%) rotateY(${i*step + x*6}deg) rotateX(${ -y*4 }deg) translateZ(${radius}px)`;
        });
        card.addEventListener('mouseleave', ()=>{
          card.style.transform = `translate(-50%,-50%) rotateY(${i*step}deg) translateZ(${radius}px)`;
        });
        card.addEventListener('click', ()=> window.location.href = f.href);
        card.addEventListener('keydown', (e)=>{ if(e.key==='Enter') window.location.href = f.href; });
